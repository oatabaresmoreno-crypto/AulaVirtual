...
@section('content')
    ...
    <div class="flex items-center justify-between mb-6">
        <h1 class="text-2xl font-bold text-gray-900">Editar Inscripción</h1>
        ...
        <form method="POST" action="{{ route('admin.enrollments.update', $enrollment) }}">
            ...
            <div class="mb-6">
                ...
                <input type="date" name="completed_at"
                       value="{{ old('completed_at', $enrollment->completed_at?->format('Y-m-d')) }}"
                       class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm
                              focus:outline-none focus:ring-2 focus:ring-indigo-500
                              @error('completed_at') border-red-400 @enderror">
                <p class="text-xs text-gray-400 mt-1">Se completa automáticamente al cambiar el estado a completado.</p>
                @error('completed_at')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
